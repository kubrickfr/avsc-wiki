...
objects. For example the `string` `PrimitiveType` knows how to
[`encode`](Api#typeencodeobj-size-unsafe) and
[`decode`](Api#typedecodebuf-resolver-unsafe) JavaScript strings:
...
an existing schema. `avsc` exposes a [`parse`](Api#parseschema-opts) method to
...
Since schemas are often stored in separate files, passing a string to `parse`
will attempt to load a JSON-serialized schema from that path:
